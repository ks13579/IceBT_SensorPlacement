...
def get_measured_temperatures(T_end,sensor_depths, sensor_std, z_grid, used_profiles):
    ...
    for p in used_profiles:
        ...
        spline = interpolate.interp1d(z_grid, T_end[:,p],kind='cubic')
...
def build_interpolants(T_end,sensor_depths, measured_temperatures,used_profiles):
    ...
    for i in range(len(used_profiles)):
        interpolants.append(interpolate.interp1d(sensor_depths, measured_temperatures[i],kind='cubic'))
    return interpolants
...
def evaluate_measurement_errors(interpolants, z_grid, error_grid, T_end, used_profiles):
    ...
    i = 0
    for p in used_profiles:
        T_end_interpolant = interpolate.interp1d(z_grid, T_end[:,p],kind='cubic')
...
for sn in rm_sensors:
    ...
    np.save('../output/Discussion/Interpolation_types/EDML_cubic_interp_%dsensor_depths_linear'%(sn),sensor_depths_linear)
    np.save('../output/Discussion/Interpolation_types/EDML_Discussion_cubic_interp_%dsensor_0derr_accumulated_error_linear'%(sn),accumulated_error_linear)
    np.save('../output/Discussion/Interpolation_types/EDML_cubic_interp_%dsensor_0derr_sensor_depths_logarthmic'%(sn),sensor_depths_logarthmic)
    np.save('../output/Discussion/Interpolation_types/EDML_cubic_interp_%dsensor_0derr_accumulated_error_logarthmic'%(sn),accumulated_error_logarthmic)
    
    if go_status == 'okay':
        np.save('../output/Discussion/Interpolation_types/EDML_cubic_interp_%dsensor_0derr_sensor_depths_go_avg'%(sn),sensor_depths_go_avg)
        np.save('../output/Discussion/Interpolation_types/EDML_cubic_interp_%dsensor_0derr_accumulated_error_go_avg'%(sn),accumulated_error_go_avg)
